...
import { sideOf, statusWord } from './Masthead';
...
export function GamesList({ games, id, loading, href }: { games: any[]; id: string; loading: boolean; href: (p: string) => string }) {
  ...
  const cols: Column<any>[] = [
    ...
    { key: 'result', label: 'Result', value: (g) => { const s = sideOf(g, id); return s.result ? (s.us ?? 0) - (s.them ?? 0) : null; }, render: (g) => { const s = sideOf(g, id); return s.result ? <ResultBadge result={s.result} us={s.us} them={s.them} ot={g.overtime} pk={g.shootout} forfeit={g.forfeit} /> : <Badge>{statusWord(g)}</Badge>; } },
          ...
          return (
            <li key={g.id}>
              <Link to={href(`/matches/${g.id}`)} className="flex items-center gap-3 p-3 hover:bg-field-800">
                ...
                {s.result ? <ResultBadge result={s.result} us={s.us} them={s.them} ot={g.overtime} pk={g.shootout} forfeit={g.forfeit} /> : <Badge>{statusWord(g)}</Badge>}
              </Link>
            </li>
          );
        })}
