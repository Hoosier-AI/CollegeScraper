...
export function StandingsGrid(props: { conference?: string; embedded?: boolean } = {}) {
                      ...
                      return (
                        <Fragment key={r.program_id}>
                          ...
                          <tr>
                            ...
                            <td className="td"><Link className="flex items-center gap-2 font-medium hover:text-pitch-300" to={href(`/teams/${r.program_id}`)}><TeamLogo src={r.college_programs?.college_schools?.logo_svg_url} seo={r.college_programs?.school_seo} name={r.college_programs?.name} size={18} /><span className="truncate">{r.college_programs?.name}</span></Link></td>
