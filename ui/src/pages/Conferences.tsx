...
import { Badge, EmptyState, ErrorBox, Field, PageHeader, SegmentedControl, Skeleton, TeamLogo } from '../components/primitives';
...
export default function Conferences() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Conferences" meta={q.data ? `${rows.length} conferences, ${scope}` : scope}>
      ...
      <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {rows.map((c) => {
          ...
          return (
            <li key={c.id}>
              <Link to={href(`/conferences/${c.id}`)} className="card block h-full p-4 transition-colors duration-150 hover:border-field-600 hover:bg-field-800/60">
