  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-black">Rankings</h1>
        ...
        <select className="input" value={q.data?.week ?? week} onChange={(e) => setWeek(e.target.value)}>{(q.data?.weeks ?? []).map((w: any) => <option key={w.week_of} value={w.week_of}>{w.label} · {fmt.date(w.week_of)}</option>)}</select>
      </div>
