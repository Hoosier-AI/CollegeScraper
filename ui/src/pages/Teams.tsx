...
export default function Teams() {
  ...
  const list = useQuery({ queryKey: ['programs', f.season, f.gender, division, conference], queryFn: () => api<ProgramRow[]>(`/api/programs${qs({ season: f.season, gender: f.gender, division, conference })}`) });
  ...
  const cols: Column<ProgramRow>[] = [
    ...
    { key: 'record', label: 'W-L-T', value: (p) => p.record?.w ?? null, render: (p) => fmt.rec(p.record?.w, p.record?.l, p.record?.t) },
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-ink-400">Click a program to open its stats. <b className="text-ink-200">Sync</b> pulls that team's roster, schedule, season stats, box scores and bios from its athletics site, then the NCAA.com box scores, and recomputes aggregates (about 1–2 minutes).</p>
      {list.isLoading && <Spinner />}
      {list.error && <ErrorBox error={list.error} />}
      {list.data && <DataTable rows={rows} columns={cols} rowKey={(p) => p.id} onRow={(p) => nav(keep(`/teams/${p.id}`))} rowHref={(p) => keep(`/teams/${p.id}`)} defaultSort={{ key: 'name', dir: 'asc' }} empty="No programs for this season yet — run discover-teams from the Jobs page." dense />}
    </div>
  );
}
