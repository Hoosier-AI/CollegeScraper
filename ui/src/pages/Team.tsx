      ...
      <div className="card flex flex-wrap items-start gap-4">
        ...
        <div className="min-w-[240px]">
          ...
          <div className="mt-1 flex flex-wrap gap-2 text-sm text-ink-400">
            ...
            {t.standingsChecks?.length > 0 && <span className="text-amber-300" title={t.standingsChecks.map((c: any) => `${c.field}: official ${c.official} vs ours ${c.computed}`).join('\n')}>{t.standingsChecks.length} record mismatch{t.standingsChecks.length > 1 ? 'es' : ''}</span>}
          </div>
