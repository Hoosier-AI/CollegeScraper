...
import { ErrorBox, Section, Spinner } from '../components/primitives';
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10 pb-6">
      <header className="space-y-3">
        <h1 className="display text-3xl text-chalk-100 sm:text-4xl">The Plaibook Stats API</h1>
        <p className="max-w-3xl text-chalk-300">
          Read-only HTTP access to every NCAA soccer program, roster, game, box score, season stat, conference
          standing and poll we hold — the same data this site renders. <strong className="text-chalk-100">No key, no
      ...
      <Section title="Authentication and limits">
        <div className="card space-y-3 p-4 text-sm text-chalk-300">
          <p>
            <strong className="text-chalk-100">Anonymous (default).</strong> Every route answers requests with no
            credentials at all, up to <strong className="text-chalk-100">{meta.data?.limits.anon_per_min ?? 60} requests a minute per IP</strong>.
            Keyless responses may be read from any origin, so browser code can call them directly.
          </p>
          <p>
            <strong className="text-chalk-100">With a key.</strong> A named key raises the limit to {meta.data?.limits.key_per_min ?? 600} a
          ...
          {meta.data?.contact && (
            <p>Need a higher limit, or planning something heavy? <a className="text-pitch-400 hover:text-pitch-300" href={`mailto:${meta.data.contact}?subject=Plaibook%20Stats%20API%20key`}>Ask for a key</a>.</p>
          )}
        </div>
      </Section>

      <Section title="Routes" right={<a className="text-sm text-pitch-400 hover:text-pitch-300" href="/v1/openapi.json">OpenAPI 3.1 document</a>}>
        {spec.isPending && <Spinner />}
        <div className="space-y-2">
          {routes.map(([path, item]) => (
            <details key={path} className="rounded-lg border border-field-700 bg-field-900">
              <summary className="cursor-pointer select-none px-3 py-2">
                <span className="mr-2 rounded bg-field-800 px-1.5 py-0.5 text-2xs font-semibold text-pitch-400">GET</span>
                <Mono>{path}</Mono>
                <span className="ml-2 text-sm text-chalk-400">{item.get.summary}</span>
              </summary>
              <div className="border-t border-field-700 px-3 py-2">
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {item.get.parameters.map((p) => (
                        <tr key={p.name}>
                          <td className="td"><Mono>{p.name}</Mono>{p.required && <span className="ml-1 text-2xs text-amber-300">required</span>}</td>
                          <td className="td text-chalk-400">{p.schema?.enum ? p.schema.enum.slice(0, 8).join(' | ') + (p.schema.enum.length > 8 ? ' | …' : '') : p.schema?.format ?? p.schema?.type ?? 'string'}</td>
                          <td className="td whitespace-normal text-chalk-400">{p.description ?? (p.in === 'path' ? 'Path segment.' : '')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ) : <p className="text-sm text-chalk-500">No parameters.</p>}
              </div>
            </details>
          ))}
        </div>
      ...
      <Section title="How records are verified">
        <div className="card space-y-2 p-4 text-sm text-chalk-300">
          <p>
...
function StatList({ label, names }: { label: string; names?: string[] }) {
  ...
  return (
    <div>
      <div className="mb-1 text-2xs font-medium text-chalk-500">{label}</div>
      <div className="flex flex-wrap gap-1">
        {names.map((n) => <span key={n} className="rounded bg-field-800 px-1.5 py-0.5 font-mono text-2xs text-chalk-300">{n}</span>)}
      </div>
    </div>
  );
}
    ...
    <div className="card space-y-3">
      ...
      {!!op?.parameters?.length && (
        <div className="flex flex-wrap gap-2">
          {op.parameters.map((p) => (
            <label key={p.name} className="flex items-center gap-1 text-xs text-chalk-400">
      ...
      {result && (
        <div className="space-y-1">
          <div className="flex flex-wrap gap-3 text-xs text-chalk-400">
          ...
          <pre className="max-h-96 overflow-auto rounded-lg border border-field-700 bg-field-950 p-3 text-2xs leading-snug text-chalk-300">{result.body}</pre>
        </div>
      )}
    </div>
  );
...
function Code({ children }: { children: string }) {
  ...
  return (
    <div className="relative">
      <pre className="overflow-auto rounded-lg border border-field-700 bg-field-950 p-3 pr-20 text-xs leading-relaxed text-pitch-300">{children}</pre>
...
const Mono = ({ children }: { children: React.ReactNode }) => <code className="rounded bg-field-800 px-1 py-0.5 font-mono text-xs text-chalk-200">{children}</code>;
