...
| `GET /v1/games/{id}?include=raw` | `game` header, `team[]` (both sources), `players[]`, `events[]`, `stats { provisional, live_stats_at }`; `raw` adds the source payloads. **Live matches:** while `game.status` is `live`, the `ncaa` rows are a snapshot of NCAA.com's in-game feed (`stats.provisional: true`, refreshed about every two minutes, `Cache-Control: max-age=15`); the final box score replaces them minutes after full time. Season totals never include provisional rows |
...
| `GET /v1/matches?date=&days=&gender=&division=&conference=&status=&only=` | Matches for a day (or up to 7 days from `date`, Eastern): `{ from, to, season, live, games[] }` ordered live first, then by kickoff. Each game carries both sides with logo, conference, latest poll rank and score, `live { period, clock, updated_at }` while in play, and `start_epoch` (unix seconds). Scores refresh every 3 minutes during games Also per game: `venue { name, city }` (ground and "City, ST" from the schools' schedules or the box score; the home side is `home`, `neutral_site` marks a neutral ground), `weather` (National Weather Service hourly forecast for the ground at kickoff — `temp_f`, `wind_mph`, `wind_gust_mph`, `wind_dir`, `precip_pct`, `humidity`, `short`, `place`, `read_at`, `time_assumed` when no kickoff time is published — refreshed every 3 hours and hourly in the last 6, kept after kickoff; null beyond the ~6-day forecast or when the ground is unknown), `result_pending` and `kickoff_tbd` |
| `GET /v1/matches/{id}/preview` | Everything around one match: head-to-head record and every previous meeting (date, score, W/L/T for the current home team, scorers with minutes), plus clean sheets, goals per game, home/away record, biggest wins and the current run, each side's season stats and form, standing (`rank` of `of`), poll rank, key players (scorers, assists, keeper) and the lineup (starters, subs, did-not-play, each with `headshot_url` when the school publishes a hotlinkable photo). `lineup` is only ever this match's own box score; `lineup_status` says what you got: `match` (starters named), `no_starters` (the box score only lists who played), `past` (no box score yet — `last_lineup` holds the side's latest lineup with starters, with `game_date` and `opponent`) or `none` |
| `GET /v1/conferences?season=&gender=&division=` | Conferences with member counts and the top three of each table |
| `GET /v1/conferences/{id}?season=&gender=` | One conference: standings, members with form and poll rank, this week's matches, leaders (points, goals, assists, save %), ranked members |
