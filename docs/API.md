...
| `GET /v1/matches/{id}/preview` | Everything around one match: head-to-head record and previous meetings, each side's season stats and form, standing (`rank` of `of`), poll rank, key players (scorers, assists, keeper) and the lineup (starters, subs, did-not-play) once a box score exists — before kickoff, the side's last lineup |
