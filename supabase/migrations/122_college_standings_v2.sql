-- College soccer: NCAA membership flags, official conference standings, full poll history, richer aggregates.
-- Depends on 120_college_soccer.sql.

-- ---------- membership ----------
ALTER TABLE public.college_program_seasons
  ADD COLUMN IF NOT EXISTS ncaa_member boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS member_source text,                 -- 'leaderboard' | 'conference' | 'none'
  ADD COLUMN IF NOT EXISTS official_w integer,
  ADD COLUMN IF NOT EXISTS official_l integer,
  ADD COLUMN IF NOT EXISTS official_t integer,
  ADD COLUMN IF NOT EXISTS official_record_at timestamptz;
CREATE INDEX IF NOT EXISTS idx_college_program_seasons_member ON public.college_program_seasons(season, ncaa_member);

-- ---------- conference websites ----------
ALTER TABLE public.college_conferences
  ADD COLUMN IF NOT EXISTS site_host text,
  ADD COLUMN IF NOT EXISTS standings_path_m text,
  ADD COLUMN IF NOT EXISTS standings_path_w text,
  ADD COLUMN IF NOT EXISTS points_rule text NOT NULL DEFAULT 'wlt-3-1-0',
  ADD COLUMN IF NOT EXISTS short_name text;

-- ---------- standings ----------
ALTER TABLE public.college_standings
  ADD COLUMN IF NOT EXISTS source text NOT NULL DEFAULT 'computed',   -- 'conference' (official site) | 'computed'
  ADD COLUMN IF NOT EXISTS source_url text,
  ADD COLUMN IF NOT EXISTS pod text,
  ADD COLUMN IF NOT EXISTS conf_pct numeric(5,3),
  ADD COLUMN IF NOT EXISTS overall_pct numeric(5,3),
  ADD COLUMN IF NOT EXISTS conf_gf integer,
  ADD COLUMN IF NOT EXISTS conf_ga integer,
  ADD COLUMN IF NOT EXISTS gf integer,
  ADD COLUMN IF NOT EXISTS ga integer,
...
ALTER TABLE public.college_standings ALTER COLUMN conf_pts TYPE numeric(6,1);

-- Official standings row vs our computed record for the same program (empty = verified).
CREATE TABLE IF NOT EXISTS public.college_standings_checks (
  ...
  field text NOT NULL,                       -- 'conf_record' | 'overall_record' | 'ncaa_record'
  official text,
  computed text,
  checked_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (season, program_id, field)
);

-- ---------- rankings ----------
ALTER TABLE public.college_rankings
  ADD COLUMN IF NOT EXISTS previous_rank integer,
  ADD COLUMN IF NOT EXISTS first_place_votes integer,
  ADD COLUMN IF NOT EXISTS record text,
  ADD COLUMN IF NOT EXISTS label text,
  ADD COLUMN IF NOT EXISTS source_url text;

-- ---------- richer aggregates ----------
ALTER TABLE public.college_player_season_stats
  ADD COLUMN IF NOT EXISTS goals_1h integer, ADD COLUMN IF NOT EXISTS goals_2h integer, ADD COLUMN IF NOT EXISTS goals_ot integer,
  ADD COLUMN IF NOT EXISTS minutes_per_goal numeric(7,1), ADD COLUMN IF NOT EXISTS shots_per_goal numeric(6,2), ADD COLUMN IF NOT EXISTS pk_pct numeric(5,3),
  ADD COLUMN IF NOT EXISTS clean_sheets integer, ADD COLUMN IF NOT EXISTS saves_p90 numeric(6,2),
  ADD COLUMN IF NOT EXISTS div_rank_points integer, ADD COLUMN IF NOT EXISTS div_rank_goals integer, ADD COLUMN IF NOT EXISTS div_rank_assists integer,
  ADD COLUMN IF NOT EXISTS conf_rank_points integer, ADD COLUMN IF NOT EXISTS conf_rank_goals integer,
  ADD COLUMN IF NOT EXISTS pct_points_p90 numeric(5,3), ADD COLUMN IF NOT EXISTS pct_goals_p90 numeric(5,3), ADD COLUMN IF NOT EXISTS pct_assists_p90 numeric(5,3),
  ADD COLUMN IF NOT EXISTS pct_shots_p90 numeric(5,3), ADD COLUMN IF NOT EXISTS pct_save_pct numeric(5,3), ADD COLUMN IF NOT EXISTS pct_gaa numeric(5,3);

ALTER TABLE public.college_team_season_stats
  ADD COLUMN IF NOT EXISTS gf_home integer, ADD COLUMN IF NOT EXISTS gf_away integer, ADD COLUMN IF NOT EXISTS ga_home integer, ADD COLUMN IF NOT EXISTS ga_away integer,
  ADD COLUMN IF NOT EXISTS gf_1h integer, ADD COLUMN IF NOT EXISTS gf_2h integer, ADD COLUMN IF NOT EXISTS ga_1h integer, ADD COLUMN IF NOT EXISTS ga_2h integer,
  ADD COLUMN IF NOT EXISTS shots_per_goal numeric(6,2), ADD COLUMN IF NOT EXISTS sog_pct numeric(5,3),
  ADD COLUMN IF NOT EXISTS pk_goals integer, ADD COLUMN IF NOT EXISTS pk_att integer,
  ADD COLUMN IF NOT EXISTS vs_ranked_w integer, ADD COLUMN IF NOT EXISTS vs_ranked_l integer, ADD COLUMN IF NOT EXISTS vs_ranked_t integer,
  ADD COLUMN IF NOT EXISTS ppg numeric(4,2), ADD COLUMN IF NOT EXISTS last5_gf integer, ADD COLUMN IF NOT EXISTS last5_ga integer,
  ADD COLUMN IF NOT EXISTS div_rank_ppg integer, ADD COLUMN IF NOT EXISTS conf_rank_ppg integer,
  ADD COLUMN IF NOT EXISTS conf_rank_gf_pg integer, ADD COLUMN IF NOT EXISTS conf_rank_ga_pg integer, ADD COLUMN IF NOT EXISTS conf_rank_shots_pg integer,
  ADD COLUMN IF NOT EXISTS div_pct_gf_pg numeric(5,3), ADD COLUMN IF NOT EXISTS div_pct_ga_pg numeric(5,3), ADD COLUMN IF NOT EXISTS div_pct_shots_pg numeric(5,3);

CREATE TABLE IF NOT EXISTS public.college_player_season_splits (
  player_season_id uuid NOT NULL REFERENCES public.college_player_seasons(id) ON DELETE CASCADE,
  split text NOT NULL,                        -- home | away | neutral | conf | nonconf | vs_ranked
  gp integer NOT NULL DEFAULT 0, gs integer NOT NULL DEFAULT 0, minutes integer,
  goals integer, assists integer, points integer, shots integer, sog integer, saves integer, ga integer,
  PRIMARY KEY (player_season_id, split)
);
...
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['college_standings_checks','college_player_season_splits'] LOOP
...
-- ---------- views (re-created so the new columns + membership flag are exposed) ----------
DROP VIEW IF EXISTS public.college_v_player_leaders;
CREATE VIEW public.college_v_player_leaders WITH (security_invoker = true) AS
SELECT ps.player_id, p.display_name, p.suppress, ps.program_id, pr.name AS program_name, pr.gender,
       s.seo AS school_seo, s.logo_svg_url, ps.season, pse.division, pse.conference_id, c.name AS conference_name, pse.ncaa_member,
...
DROP VIEW IF EXISTS public.college_v_team_leaders;
CREATE VIEW public.college_v_team_leaders WITH (security_invoker = true) AS
SELECT pr.name AS program_name, pr.gender, s.seo AS school_seo, s.logo_svg_url,
       pse.division, pse.conference_id, c.name AS conference_name, pse.ncaa_member, ts.*
...
GRANT SELECT ON public.college_v_player_leaders, public.college_v_team_leaders TO authenticated, service_role;

-- The full-season aggregate refresh takes ~10 s on a D1+D2+D3 season, over PostgREST's 8 s default
-- (inherited from the authenticator role). Only the service role (the CollegeScraper service) gets more time.
ALTER ROLE service_role SET statement_timeout = '300s';
NOTIFY pgrst, 'reload config';

-- ---------- aggregates RPC (replaces the 120 version) ----------
...
BEGIN
  ...
  WITH tg AS (
    SELECT g.id AS game_id, side.program_id, g.season, g.game_date, side.is_home, g.neutral_site, g.conference_game, g.attendance,
           ...
           CASE WHEN side.is_home THEN g.away_score ELSE g.home_score END AS ga,
    ...
    SELECT program_id, sum(gf) AS gf, sum(ga) AS ga FROM (SELECT program_id, gf, ga, row_number() OVER (PARTITION BY program_id ORDER BY game_date DESC) AS rn FROM res) x WHERE rn <= 5 GROUP BY program_id
  ), halves AS (
    ...
    SELECT program_id, count(*) AS gp, count(*) FILTER (WHERE shots IS NOT NULL) AS box_gp,
      ...
      count(*) FILTER (WHERE r='W' AND vs_ranked) AS vs_ranked_w, count(*) FILTER (WHERE r='L' AND vs_ranked) AS vs_ranked_l, count(*) FILTER (WHERE r='T' AND vs_ranked) AS vs_ranked_t,
      ...
      sum(gf) FILTER (WHERE is_home AND NOT neutral_site) AS gf_home, sum(gf) FILTER (WHERE NOT is_home AND NOT neutral_site) AS gf_away,
      sum(ga) FILTER (WHERE is_home AND NOT neutral_site) AS ga_home, sum(ga) FILTER (WHERE NOT is_home AND NOT neutral_site) AS ga_away,
  ...
  SELECT a.program_id, p_season, a.gp, a.w, a.l, a.t, a.conf_w, a.conf_l, a.conf_t, a.home_w, a.home_l, a.home_t, a.away_w, a.away_l, a.away_t,
      ...
      round(a.gf::numeric / a.gp, 2), round(a.ga::numeric / a.gp, 2), round(a.shots::numeric / nullif(a.box_gp, 0), 2), round(a.sog::numeric / nullif(a.box_gp, 0), 2), round(a.corners::numeric / nullif(a.box_gp, 0), 2),
      ...
      round((3 * a.w + a.t)::numeric / a.gp, 2), l5.gf, l5.ga, now()
