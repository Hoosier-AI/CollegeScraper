...
export function openapiSpec(serverUrl: string): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      '/v1/status': { get: { summary: 'Crawl health', responses: ok('Recent crawl runs, quality-check counts and standings/rankings freshness.') } },
      ...
      '/v1/leaders': { get: { summary: 'Leaderboards', parameters: [season, gender, division, conference, { name: 'kind', in: 'query', schema: { type: 'string', enum: ['player', 'team'] } }, { name: 'stat', in: 'query', schema: { type: 'string', enum: [...PLAYER_STATS, ...TEAM_STATS] } }, { name: 'min_minutes', in: 'query', schema: { type: 'integer' } }, { name: 'limit', in: 'query', schema: { type: 'integer', maximum: 500 } }, { name: 'members', in: 'query', schema: { type: 'string', enum: ['all'] } }], responses: ok('{ stats: allowed stat names, rows[] } sorted by the stat (ascending for goals-against style stats).') } },
