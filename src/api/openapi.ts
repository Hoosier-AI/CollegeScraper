...
export function openapiSpec(serverUrl: string): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      '/v1/matches': { get: { summary: 'Matches by day: live, upcoming and finished', parameters: [{ name: 'date', in: 'query', schema: { type: 'string', format: 'date' }, description: 'Eastern date; defaults to today' }, { name: 'days', in: 'query', schema: { type: 'integer', minimum: 1, maximum: 7, default: 1 } }, gender, division, conference, { name: 'status', in: 'query', schema: { type: 'string', enum: ['scheduled', 'live', 'final', 'postponed', 'cancelled'] } }, { name: 'only', in: 'query', schema: { type: 'string', enum: ['conf', 'ranked'] }, description: 'conference games only, or games with a ranked team' }], responses: ok('{ from, to, season, live, games[], generated_at }. Games are ordered live first, then by kickoff (start_epoch, unix seconds). Each game has home/away { program_id, name, short_name, seo, logo, conference, division, rank (latest coaches poll), score }, live { period, clock, updated_at } while in play, venue, flags (neutral_site, conference_game, postseason, forfeit, overtime, shootout). Scores refresh every 3 minutes during games; responses with a live game are cached for 15 s.') } },
      '/v1/matches/{id}/preview': { get: { summary: 'Match preview and review: head-to-head, form, standing, poll rank, key players, lineups', parameters: [{ name: 'id', in: 'path', required: true, schema: uuid }], responses: ok('{ game, head_to_head { played, home_wins, away_wins, ties, home_goals, away_goals, games[] }, sides { home, away } } — each side: stats (team season stats; form.last5 oldest→newest, streak), standing (rank, pod, conf record, points, of = table size), poll { rank, week_of }, leaders { scorers[], assists[], keeper }, lineup_status (match: this match\'s box score names starters · no_starters: its box score only lists who played · past: no box score yet · none), lineup { source, starters[], subs[], dnp[], keeper } for this match only (null until its box score exists), last_lineup { …, game_id, game_date, opponent } the side\'s latest lineup with starters when lineup is null; each line carries headshot_url where the school publishes a hotlinkable photo. Suppressed players are withheld (numbers stay, names and ids go).') } },
      '/v1/conferences': { get: { summary: 'Conferences with member counts and the top of each table', parameters: [season, gender, division], responses: ok('{ season, conferences: [{ id, ncaa_seo, name, short_name, division, site_host, members { m, w }, table { m|w: { top: [{ program_id, name, seo, logo, rank, conf_w, conf_l, conf_t, conf_pts }], source: conference|computed|none } } }] } sorted by name.') } },
      '/v1/conferences/{id}': { get: { summary: 'One conference: table, members, this week\'s matches, leaders, ranked members', parameters: [{ name: 'id', in: 'path', required: true, schema: uuid }, season, { ...gender, description: 'm (default) or w' }], responses: ok('{ conference, season, gender, standings (as /v1/standings for one conference), members[] (with season stats, form and poll rank), this_week (as /v1/matches, Monday to Sunday Eastern), leaders { points[], goals[], assists[], save_pct[] }, ranked[] }.') } },
