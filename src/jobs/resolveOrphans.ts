...
export async function resolveOrphans(ctx: JobContext): Promise<void> {
  ...
  for (const [k, list] of byProgDate) {
    ...
    for (const dup of live.filter((x) => x !== canon && !x.ncaa_contest_id)) {
      ...
      // Programs do not play twice in a day: an unlinked final next to an NCAA-listed one is a duplicate or a line
      // whose opponent resolved to the wrong school ("Loyola" → Loyola Chicago for Lehigh's opponent). When the result
      // agrees (or is the same pair reversed) the canonical keeps its site refs for a fresh box-score fetch.
      const sameResult = df == null || cf == null || (df === cf && da === ca) || (df === ca && da === cf);
      if (sameResult) {
